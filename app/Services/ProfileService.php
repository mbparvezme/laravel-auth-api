...
class ProfileService
{
    ...
    public function update(Request $request)
    {
        ...
        $previousData = $profile->only(array_keys($validated));
        $user->name = $validated['name'];

        DB::transaction(function () use ($user, $profile, $validated, $previousData) {
            $user->save();
            $profile->update($validated);
            $this->addLog(
                action: self::$logKey['profile_updated'],
                data: ['previous' => $previousData, 'new' => $validated,],
                user: $user->id
            );
        });
    ...
    public function accountStatus($status, Request $request)
    {
